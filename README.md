...
# Realm for Hermes (required — regular realm won't work with drawer nav)
...
**Realm + Hermes** was a wall I hit early. The regular `npm install realm` doesn't work with Drawer Navigation because Drawer requires the Reanimated library, which uses Hermes — and the standard Realm build isn't compatible. Finding out that `realm@hermes` exists and that I had to also modify `MainApplication.java` manually was not something I would have worked out without digging. Actually understanding *why* those steps were needed made the fix stick.

**The update logic** in `EditProductScreen` was genuinely tricky to reason about. You have data coming from Realm, displayed in controlled inputs, and you need to figure out whether anything changed before deciding whether to write. The condition that checks every field of `updatedData` against `productData` before saving felt like writing real application logic — not just following a pattern.
